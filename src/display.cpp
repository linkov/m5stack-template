...
//Arduino_ILI9341 *gfx = new Arduino_ILI9341(bus, 33 /* RST */, 1 /* rotation */);
...
extern "C" void display_write_frame(const uint8_t *data[])
{
    ...
    if (w < 480)
    {
        ((Arduino_TFT *)gfx)->writeAddrWindow(frame_x, frame_y, frame_width, frame_height);
        for (int32_t i = 0; i < NES_SCREEN_HEIGHT; i++)
        {
            ((Arduino_TFT *)gfx)->writeIndexedPixels((uint8_t *)(data[i] + frame_x_offset), myPalette, frame_line_pixels);
        }
    }
    else
    {
        ...
        for (int16_t i = 10; i < (10 + 214); i++)
        {
            ((Arduino_TFT *)gfx)->writeAddrWindow(frame_x, y++, frame_width, 1);
            ((Arduino_TFT *)gfx)->writeIndexedPixelsDouble((uint8_t *)(data[i] + 8), myPalette, frame_line_pixels);
            if ((i % 2) == 1)
            {
                y++; // blank line
            }
